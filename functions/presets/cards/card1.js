...
module.exports = async (data, client, player, emoji) => {
  const color = "#aaaaaa";
  ...
  const requester = data.requester.username;
  ...
  const embed = new client.embed()
    .desc(`<:Music:1253270060492128288> • Now Playing **${
      title.charAt(0).toUpperCase() +
      title.substring(0, 25).slice(1).toLowerCase()
    }**\n`)
    .img(`attachment://${attachment.name}`)
    .setFooter({
      text: `Requested by ${requester} • (${duration})`,
    });
